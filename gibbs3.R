gibbs3 = function(x,K,d,niter,a,g,burnin){
  ...
  #####################post###################################
  post <- function(loglik,eta,z,mu,Sigmamns1,G0,a,g,h){
    lpost=numeric(1)
    num <- numeric(K)
    for (i in 1:K){
      num[i] <- sum(z==i)
    }
    lpost <- lpost + sum(dmvnorm(mu,m0,M0,log=TRUE))
                   + log(dwish(G0,2*g,solve(2*h))) + log(ddirichlet(eta,delta+num))
    for (j in 1:K){
      lpost <- lpost + sum(z==j)*log(eta[1,j]) + log(dwish(Sigmamns1[,((j-1)*d+1):(j*d)],2*a,solve(2*G0)))
    }
    lpost <- loglik + lpost
    return(lpost)
  }
  ...
  post.data <- post(loglik=loglikhood.data,eta=eta,z=z,mu=mu,Sigmamns1=Sigmamns1,G0=G0,a=a,g=g,h=h)
