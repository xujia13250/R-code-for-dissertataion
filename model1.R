gibbs1 = function(x,K,d,niter,a,g,burnin){
  ...
  omega <- apply(x,2,median)
  Omega <- diag(c((max(x[,1]) - min(x[,1]))^(-2),(max(x[,2]) - min(x[,2]))^(-2)),nrow=d)
  h <- 100*g*Omega/a
  mu <- rmvnorm(K,omega,solve(Omega))
...
result1 <- gibbs1(x=data3,K=K,d=d,niter=niter,burnin=burnin,a=a,g=g) 
...
burnin=2000
begin.time <- proc.time()
result2 <- gibbs1(x=data3,K=K,d=d,niter=niter,burnin=burnin,a=a,g=g) 
proc.time() - begin.time
...
burnin=2000
begin.time <- proc.time()
result3 <- gibbs1(x=data3,K=K,d=d,niter=niter,burnin=burnin,a=a,g=g) 
proc.time() - begin.time
...
df1 <- data.frame(index=rep(1:(niter+burnin-1)),
                 AIC.2=result1$AIC,AIC.3=result2$AIC,AIC.4=result3$AIC)
didfm1 <- melt(df1,id.vars="index")
ggplot(didfm1,aes(x=index,y=value)) + geom_line(aes(color=variable)) + ggtitle("AIC for different number of components")


df2 <- data.frame(index=rep(1:(niter+burnin-1)),
                 BIC.2=result1$BIC,BIC.3=result2$BIC,BIC.4=result3$BIC)
didfm2 <- melt(df2,id.vars="index")
ggplot(didfm2,aes(x=index,y=value)) + geom_line(aes(color=variable)) + ggtitle("BIC for different number of components")


median(exp(result2$loglikhood)/exp(result3$loglikhood))
median(exp(result1$loglikhood)/exp(result2$loglikhood))
...
mu.df.1 <- mu.trans(result1$mu,2)
ggplot(mu.df.1,aes(x=mu.df.1$x.1,y=mu.df.1$x.2)) + geom_point(aes(colour=z)) +
  scale_colour_gradientn(colours=brewer.pal(n = 2, name = "Pastel2")) + 
   ggtitle("Estimate of mu for K=2") + xlab("mu1") +ylab("mu2") 


mu.df.2 <- mu.trans(result2$mu,2)
ggplot(mu.df.2,aes(x=x.1,y=x.2)) + geom_point(aes(colour=z)) +
  scale_colour_gradientn(colours=brewer.pal(n =3, name = "Pastel2")) + 
  ggtitle("Estimate of mu for K=3") + xlab("mu1") +ylab("mu2") 

mu.df.3 <- mu.trans(result3$mu,2)
ggplot(mu.df.3,aes(x=x.1,y=x.2)) + geom_point(aes(colour=z)) +
  scale_colour_gradientn(colours=brewer.pal(n =4, name = "Pastel2")) + 
  ggtitle("Estimate of mu for K=4") + xlab("mu1") +ylab("mu2") 



df4 <- data.frame(index=rep(1:niter),
                  prob=result1$eta)
didfm3 <- melt(df4,id.vars="index")
ggplot(didfm3,aes(x=index,y=value)) + geom_line(aes(color=variable)) + ggtitle("trace plot for weights of each components(K=2)")

df5 <- data.frame(index=rep(1:niter),
                  prob=result2$eta)
didfm4 <- melt(df5,id.vars="index")
ggplot(didfm4,aes(x=index,y=value)) + geom_line(aes(color=variable)) + ggtitle("trace plot for weights of each components(K=3)")

df6 <- data.frame(index=rep(1:niter),
                  prob=result3$eta)
didfm5 <- melt(df6,id.vars="index")
ggplot(didfm5,aes(x=index,y=value)) + geom_line(aes(color=variable)) + ggtitle("trace plot for weights of each components(K=4)")
