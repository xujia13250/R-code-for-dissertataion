...
credibleball_v2=function(c.star,cls.draw,c.dist=c("VI","Binder","VI.lb"),alpha=0.05,psm=NULL){
	...
	if(c.dist=="VI.lb") d=apply(cls.draw,1,VI.lb,psm=psm)
	...
	output=list(dist.horiz=sd$x[ind.star])
	return(output)
}
